...
/** Horizontal probability bar, tier-colored (single source of truth:
 * getRiskTierColor, the same resolver every badge and chart uses), with
 * the exact percentage in tabular mono type. Tick marks at the real
 * decision-tier boundaries let an analyst see at a glance not just "how
 * risky" but "how close to flipping tiers." */
export function RiskMeter({ probability, tier, compact = false }: RiskMeterProps) {
  ...
  return (
    <div className={`flex items-center gap-3 ${compact ? 'min-w-35' : 'min-w-55'}`}>
      <div className="relative flex-1 overflow-hidden rounded-full bg-bg-surface-raised" style={{ height: compact ? 6 : 10 }}>
        <div
        ...
        <div className="absolute top-0 h-full w-px bg-border" style={{ left: `${MEDIUM_THRESHOLD_PERCENT}%` }} />
        <div className="absolute top-0 h-full w-px bg-border" style={{ left: `${HIGH_THRESHOLD_PERCENT}%` }} />
      </div>
      <span className={`shrink-0 font-mono tabular-nums ${compact ? 'text-xs' : 'text-sm font-medium'}`} style={{ color }}>
