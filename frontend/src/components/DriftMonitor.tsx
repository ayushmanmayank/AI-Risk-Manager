...
function DriftFeatureRow({ feature }: { feature: FeatureDriftOut }) {
  ...
  return (
    <div className="flex items-center gap-3">
      <div className="w-36 shrink-0 text-sm text-text-primary">
        {FEATURE_LABEL[feature.feature] ?? feature.feature}
      </div>
 ...
 * Performance rather than its own nav page -- see the Tier 3B report for
 * why (a lower-priority, illustrative capability shouldn't claim equal
 * nav real estate next to the four workflow pages this product is
 * actually built around). PSI method, thresholds, and every honesty
 * caveat live in src/monitoring/drift_detector.py -- this component only
 * renders what that report already decided. */
export function DriftMonitor({ report }: { report: DriftReportOut }) {
  return (
    <div className="space-y-6 border-t border-border pt-8">
      <div>
        <h1 className="font-display text-lg font-semibold text-text-primary">
          Model Drift Monitoring (Tier 3B)
        </h1>
      ...
      <div className="rounded-(--radius-card) border-2 border-text-primary bg-bg-surface-raised p-4 text-sm text-text-primary">
        ...
        of the capability (the plumbing from training baseline to live comparison to this badge
        genuinely works), not a production-grade drift signal. PSI thresholds below are a
        standard, widely-used convention, not tuned against this project&apos;s own data.
      </div>
      ...
      {report.insufficient_data ? (
        ...
        <div className="card-dark p-6">
          ...
          <p className="mt-1 text-xs text-text-muted">
            ...
            rows. Live window: last {report.live_sample_size.toLocaleString()} scored predictions.
            PSI &lt; 0.10 stable, 0.10-0.25 moderate, &gt;= 0.25 significant.
          </p>
          <div className="mt-5 space-y-3">
            {report.features.map((feature) => (
              <DriftFeatureRow key={feature.feature} feature={feature} />
            ))}
          </div>
        </div>
      )}
