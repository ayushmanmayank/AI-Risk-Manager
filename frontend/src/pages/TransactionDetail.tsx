  ...
  return (
    <Tooltip>
      <TooltipTrigger asChild>
        <li className="flex cursor-default items-baseline justify-between gap-4 rounded-md px-1 py-1.5 text-sm transition-colors duration-100 hover:bg-bg-surface-raised">
          <span className="text-text-primary">{contribution.label}</span>
          <span className="shrink-0 font-mono text-xs tabular-nums text-text-muted">value {contribution.feature_value.toFixed(3)}</span>
        </li>
      </TooltipTrigger>
...
export function TransactionDetail() {
  ...
  if (result.status === 'error') {
    if (result.error instanceof ApiError && result.error.status === 404) {
      return <EmptyBlock title="Transaction not found" description={`No scored transaction matches id "${id}". Check the id and try again.`} />;
    }
...
function TransactionDetailBody({ data }: { data: PredictionOut }) {
  ...
  const tierColor = getRiskTierColor(mode)[data.risk_tier];

  return (
    <div className="space-y-8">
      <div className="card p-6">
        <Link to="/high-risk" className="rounded text-sm text-text-secondary hover:text-accent focus-visible:outline-2 focus-visible:outline-accent focus-visible:outline-offset-2">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        {/* Hero single-value metric -> radial ring. Sweeps in + counts up
            on load, driven by the real fraud_probability either way. */}
        <div className="card flex items-center gap-4 p-4">
          <RadialRing
            ...
            strokeWidth={7}
            label={
              <span className="font-mono text-sm font-semibold tabular-nums" style={{ color: tierColor }}>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <div className="card p-6">
          <h2 className="font-display text-base font-semibold text-accent-rose">Top risk factors</h2>
        ...
        <div className="card p-6">
          <h2 className="font-display text-base font-semibold text-text-secondary">Risk-reducing factors</h2>
