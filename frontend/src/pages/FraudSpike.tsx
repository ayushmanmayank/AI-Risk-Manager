...
// during a live demo. 3s balances feeling live against hammering the
// backend; a manual Refresh button is also provided.
const POLL_INTERVAL_MS = 3000;

// How many recent poll readings the live sparkline keeps on screen --
// ~1 minute of real history at the 3s interval above. Purely a
// client-side display buffer of values the page already fetches on its
// existing poll -- no new API calls or backend data, it just stops
...
export function FraudSpike() {
  ...
  const bannerColor = getLevelColor(mode, SEVERITY_LEVEL[data.severity]);
  ...
  return (
    <div className="space-y-8">
      <div className="card flex flex-wrap items-start justify-between gap-4 p-6">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-sm text-text-secondary">
            Live view of the rolling fraud-rate anomaly detector: compares the HIGH-risk rate among the last{' '}
            {data.window_size || 'N'} scored transactions against the model's baseline HIGH-tier rate from
            training data. Auto-refreshes every {POLL_INTERVAL_MS / 1000}s.
          </p>
          {poll.isStale && <p className="mt-1 text-xs text-accent-rose">Last refresh failed -- showing the previous reading.</p>}
        </div>
        <button
          onClick={poll.refetch}
          className="pill-glow shrink-0 rounded-full border border-border px-4 py-2 text-sm font-medium text-text-primary hover:bg-bg-surface-raised focus-visible:outline-2 focus-visible:outline-accent focus-visible:outline-offset-2"
        >
          Refresh now
        </button>
      </div>

      {/* Calm / active-spike status banner -- a left-border treatment
          rather than a large color fill, colored by severity. */}
      <div
        className="rounded-xl border-l-4 bg-bg-surface p-6 transition-colors duration-200"
        style={{ borderLeftColor: bannerColor }}
      >
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <p className="text-xl font-semibold" style={{ color: bannerColor }}>
            ...
            {data.insufficient_data && (
              <p className="mt-1 text-xs text-text-muted">
                Only {data.window_size} prediction{data.window_size === 1 ? '' : 's'} scored so far -- not enough
                history yet for a confident read (needs at least 10).
              </p>
            )}
          </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard label="Current fraud rate" value={formatRate(data.current_fraud_rate)} caption={`last ${data.window_size} predictions`} />
        <StatCard label="Baseline fraud rate" value={formatRate(data.baseline_fraud_rate)} caption="training-set HIGH-tier rate" />
      ...
      <div className="card p-6">
        ...
        <div className="mt-3">
          <Sparkline points={history} color={palette.accent} />
        </div>
      </div>

      <div className="card p-6">
        <h2 className="font-display text-base font-semibold text-text-primary">Current vs. baseline fraud rate</h2>
        <FraudRateComparisonChart currentRate={data.current_fraud_rate} baselineRate={data.baseline_fraud_rate} severity={data.severity} />
      </div>
